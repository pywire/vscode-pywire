...
async function showUpdateNotification(cwd: string, newVersion: string, currentVersion: string) {
    ...
    if (selection === 'Upgrade') {
        ...
        await vscode.workspace.getConfiguration('pywire').update('disableUpdateNotifications', true, vscode.ConfigurationTarget.Global);
...
export async function performUpdate(cwd?: string) {
    ...
    vscode.window.withProgress({
        ...
        try {
            ...
            if (stderr.includes('resolution failed') || stderr.includes('conflict') || stderr.includes('constraint')) {
                vscode.window.showWarningMessage('PyWire update failed due to version constraints. Please check your pyproject.toml.');
            } else {
